...
function runLighthouse(url = "http://localhost:3000") {
  ...
  if (!fs.existsSync(outputDir)) {
    log(`📁 Creating directory: ${outputDir}`, "cyan");
    fs.mkdirSync(outputDir, { recursive: true });
  }

  const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
  const reportName = `lighthouse-${timestamp}`;
  const reportPath = path.join(outputDir, `${reportName}.report.html`);
  const jsonPath = path.join(outputDir, `${reportName}.report.json`);
  ...
  try {
    ...
    const lighthouseCmd =
      `lighthouse "${url}" ` +
      `--output=html,json ` +
      `--output-path="${path.join(outputDir, reportName)}" ` +
      `--only-categories=performance ` +
      `--chrome-flags="--headless --no-sandbox --disable-gpu" ` +
      `--quiet`;

    log(`Running: ${lighthouseCmd}`, "cyan");
    execSync(lighthouseCmd, { stdio: "inherit" });

    // Wait a bit for file to be written
    let retries = 0;
    while (!fs.existsSync(jsonPath) && retries < 10) {
      log(`⏳ Waiting for report file... (${retries + 1}/10)`, "yellow");
      execSync("sleep 1");
      retries++;
    }
    ...
    // Read JSON report
    log(`📖 Reading report: ${jsonPath}`, "cyan");
