...
class WebSocketServer {
  ...
  start(httpServer = null) {
    if (httpServer) {
      ...
      this.server.listen(this.port, () => {
        console.log(
          `✅ WebSocket Server started on ws://localhost:${this.port}/ws`
        );
      });
    }
  ...
  setupEventHandlers() {
    this.wss.on("connection", (ws, req) => {
      ...
      console.log(
        `🔌 Client connected: ${clientId} (${this.stats.activeConnections} active)`
      );
  ...
  handleDisconnect(clientId) {
    ...
    console.log(
      `🔌 Client disconnected: ${clientId} (${this.stats.activeConnections} active)`
    );
  }
